...
import com.example.mazadat.utils.ToastCreator;
...
public class Validation {
    ...
    private static String EMAIL_PATTERN = "^[_A-Za-z0-9-%+]+(\\.[_A-Za-z0-9-%+]+)*@[_A-Za-z0-9-%+]+(\\.[_A-Za-z0-9-%+]+)*(\\.[A-Za-z]{2,})$";
...
//        }
    ...
    public static boolean validationEmail2(Activity activity, EditText email) {
    ...
    public static boolean validationPassword2(EditText password, int length, String errorText) {
    ...
    public static boolean validationEmailLength(TextInputLayout text, String errorText, int length) {
        if (text.getEditText().length() < length) {
...
//        if (phone1.length() >= country1.getLength_min() && phone.getEditText().getText().length() <= country1.getLength_max()) {
        if (phone.getEditText().getText().length() == 11){
            return true;
        } else {
            phone.setError(activity.getString(R.string.invalid_phone1));
            return false;
    ...
    public static boolean validationAllEmpty2(List<EditText> editTexts , List<Spinner> spinners, String errorText) {
        List<Boolean> booleans = new ArrayList<>();
        for (int i = 0; i < editTexts.size(); i++) {
            if (!validationLength(editTexts.get(i), errorText)) {
                booleans.add(false);
    ...
    public static void showCookieMsg2(String title, String msg, Activity activity, int color, int popUpPosition) {
